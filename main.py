...
def draw_score():
    points_text = font.render(f'Poinst: {points}', True, 'black')
...
def draw_menu():
    global game_over, pause, mode, level, menu, time_passed, total_shots, points, ammo, time_remaining, write_values, best_time, best_freeplay, best_ammo, clicked, new_coords
    game_over = False
    pause = False
...
def draw_game_over():
    global clicked, level, pause, game_over, menu, total_shots, time_remaining, time_passed, points
    if mode == 0:
...
def draw_pause():
    global level, pause, menu, points, time_passed, time_remaining, total_shots, new_coords
